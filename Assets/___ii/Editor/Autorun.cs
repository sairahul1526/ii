...
namespace AppAdvisory.ii
{
	...
	public class Autorun
	{
		...
		static void RunOnce() 
		{
			...
			int count = EditorPrefs.GetInt(Welcomeii.PREFSHOWATSTARTUP + "autoshow",0);
			...
			EditorPrefs.SetInt(Welcomeii.PREFSHOWATSTARTUP + "autoshow", count + 1);

			Welcomeii.showAtStartup = EditorPrefs.GetInt(Welcomeii.PREFSHOWATSTARTUP, 1) == 1;
			 
			if (Welcomeii.showAtStartup)
			{
				DGChecker.CheckItNow();
			
				Welcomeii.OpenPopupStartup();
			}
		}
	}
}         